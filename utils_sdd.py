...
class DataSet_bytrajec2_sdd(Dataset):
    ...
    def is_close_enough(self, traj1, traj2):
        ...
        for i in range(seq_len):
            for j in range(seq_len):
                if abs(traj1[i, 0] - traj2[j, 0]) < 100 and abs(traj1[i, 1] - traj2[j, 1]) < 100:
                    return True
        return False
    ...
    def get_edge_info(self, setname):
        ...
        for data in databatch: # 循环大batch
            ...
            for (ffrom, tto) in batch_split: # 循环小batch
                ...
                edge_pair[(ffrom, tto)] = torch.tensor(edgeN).cuda().long()
    ...
    #     return batch_data, batch_split, nei_lists, self.train_edge_index[idx]
    ...
    #     batch_data = self.rotate_shift_batch(batch_data,epoch,idx,ifrotate=False)
    #     return batch_data, batch_split, nei_lists, self.test_edge_index[idx]
