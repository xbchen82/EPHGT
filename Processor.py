...
class Processor():
    ...
    def train_epoch(self,epoch):
        ...
        for batch, (inputs_gt, batch_split, nei_lists, edge_pair) in enumerate(self.trainloader_gt):
            ...
            inputs_fw = batch_abs_gt, batch_norm_gt, nei_lists, nei_num, batch_split, shift_value_max  #[H, N, 2], [H, N, 2], [B, H, N, N], [N, H]
            self.net.zero_grad()

            
            (EPHGT_loss, loss1), full_pre_tra = self.net.forward(inputs_fw, edge_pair, epoch, iftest=False)

            tot_loss = EPHGT_loss + loss1
            ...
            if batch%self.args.show_step == 0 and self.args.ifshow_detail:
                print('train-{}/{} (epoch {}), EPHGT_loss = {:.5f}, loss1 = {:.5f}, time/batch = {:.5f}'.\
                format(batch,len(self.trainloader_gt), epoch, EPHGT_loss.item(),
                        loss1.item(),
                        end - start))
    ...
    #         EPHGT_loss,full_pre_tra= self.net.forward(inputs_fw, epoch, iftest=True)
    #         if EPHGT_loss == 0:
    #             continue
